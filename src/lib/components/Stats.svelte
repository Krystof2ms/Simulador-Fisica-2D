<script lang="ts">
  ...
  import { exportToCSV, type ExportOptions } from '../utils/export';
...
>
  ...
  {#if !isCollapsed}
    <div class="w-107.5 bg-white border border-slate-200 rounded-2xl flex flex-col items-stretch shadow-lg overflow-hidden animate-in fade-in slide-in-from-right-4 duration-300">
      ...
      <div class="grid grid-cols-3 p-2 bg-slate-100 border-b border-slate-200 text-sm font-semibold">
        ...
        <button onclick={() => activeTab = 'export'} class="py-1.5 rounded-lg transition-all cursor-pointer focus:outline-none {activeTab === 'export' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:bg-slate-200/50 hover:text-slate-700'}">Exportar</button>
      </div>
        ...
        {:else}
          <div class="flex flex-col gap-4">
            <div class="p-4 bg-slate-50 rounded-xl border border-slate-100 shadow-inner">
              <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-3">Series a Exportar</span>
              <div class="flex flex-col gap-3">
                <label class="flex items-center gap-3 text-sm font-semibold text-slate-700 cursor-pointer">
                  <input type="checkbox" bind:checked={exportOptions.distance} class="w-4 h-4 rounded border-slate-300 text-cyan-600 focus:ring-cyan-500" />
                  Distancia y Posición
                </label>
                <label class="flex items-center gap-3 text-sm font-semibold text-slate-700 cursor-pointer">
                  <input type="checkbox" bind:checked={exportOptions.velocity} class="w-4 h-4 rounded border-slate-300 text-cyan-600 focus:ring-cyan-500" />
                  Velocidad (X, Y, Total)
                </label>
                <label class="flex items-center gap-3 text-sm font-semibold text-slate-700 cursor-pointer">
                  <input type="checkbox" bind:checked={exportOptions.acceleration} class="w-4 h-4 rounded border-slate-300 text-cyan-600 focus:ring-cyan-500" />
                  Aceleración (X, Y, Neta)
                </label>
                <label class="flex items-center gap-3 text-sm font-semibold text-slate-700 cursor-pointer">
                  <input type="checkbox" bind:checked={exportOptions.friction} class="w-4 h-4 rounded border-slate-300 text-cyan-600 focus:ring-cyan-500" />
            ...
            <div class="flex flex-col gap-3">
              ...
              <p class="text-[11px] text-slate-400 font-medium leading-relaxed px-1">
                Seleccione los datos que desea incluir en el archivo CSV. El archivo incluirá una fila por cada paso de tiempo de la simulación.
              </p>
            </div>
          </div>
        {/if}
