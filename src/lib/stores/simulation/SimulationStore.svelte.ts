...
export class SimulationStore {
    ...
    segments = $derived(
        buildTerrainSegments(this.terrainPoints)
    );
    ...
    resetSimulation() {
        ...
        const startX = Math.max(
            ...
            Math.min(
                this.canvasWidth,
                this.initialVehicleState.position.x
            )
        );
    ...
    step(dt: number) {
        ...
        if (
            this.vehicle.position.x <= leftLimit ||
            this.vehicle.position.x >= rightLimit
        ) {
            this.vehicle = {
                ...this.vehicle,
                position: {
                    x: Math.max(
                        leftLimit,
                        Math.min(rightLimit, this.vehicle.position.x)
                    ),
    ...
    scrubTo(targetTime: number) {
        targetTime = Math.max(
            0,
            Math.min(this.maxTime, targetTime)
        );

        this.isPlaying = false;

        const frameIndex = Math.round(
            targetTime / this.config.fixedDt
        );

        const frame = getHistoryFrame(
            this.history,
            frameIndex
        );
    ...
    smoothTerrain(
        ...
        this.terrainPoints = smoothTerrain(
            this.terrainPoints,
            centerX,
            radius
        );
    ...
    setInitialVehiclePosition(
        ...
        const clampedX = Math.max(
            0,
            Math.min(this.canvasWidth, x)
        );

        const clampedY = Math.max(
            this.minTerrainY,
            Math.min(this.floorLimitY, y)
        );
    ...
    updatePointPosition(
        ...
        if (
            index < 0 ||
            index >= this.terrainPoints.length
        ) {
            return;
        }
    ...
    addPoint(x: number, y: number) {
        ...
        if (insertIndex === -1) {
            ...
            const prevFriction =
                this.terrainPoints[insertIndex - 1]
                    ?.friction ?? 0.9;
